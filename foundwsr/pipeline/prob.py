...
class prob(BasePipe):
    def __init__(self, args):
        ...
        self.classifier_optimizer = self.candidate_optimizer[args.optimizer](self.classifier.parameters(),
                                                                            lr=1e-3, weight_decay=args.weight_decay)
        self.classifier_scheduler = ReduceLROnPlateau(self.classifier_optimizer, 'min', factor=0.5, patience=2, verbose=True, min_lr=1e-6)

        self.SNR_list = dataset.SNR_list
        self.output_dir = args.output_dir
        # self.checkpoint = osp.join(self.args.output_dir,
        #                                             f"{self.args.model}_{self.args.dataset[0]}_prob.pt")
    ...
    def train(self):
        stopper = EarlyStopping(10, self._checkpoint, self.args.compile_flag, self.args.use_distribute)
