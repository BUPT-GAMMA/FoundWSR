...
def get_stft_input(x):
    STFTs = []
    for sample in x:
        _, _, stp = stft(sample[0, :], 1.0, 'blackman', 31, 30, 128)
        STFTs.append(np.expand_dims(stp[:32, :], 0))

    return torch.Tensor(np.array(STFTs))

@register_pipe("iqformer_trainer")
class IQFormerTrainer(BasePipe):
    def __init__(self, args):
        super(IQFormerTrainer, self).__init__(args)
    ...
    def _train_step(self):
        ...
        for _, data in enumerate(tqdm(self.train_loader)):
            ...
            batch_stft = get_stft_input(batch_x).to(self.device)
